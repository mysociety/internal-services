...
sub write_js {
    ...
    if (defined($self->{'output'}{'error'})) {
        ...
        foreach my $id (sort keys %{$self->{'programmes'}}) {
            ...
            my $embed_html_version = $self->get_video_embed_html($id, 'full');
            my $embed_js_version = $self->get_video_embed_html($id, 'min');
            ...
            print "document.write(\"$embed_html_version\");\n";
...
sub get_programme_listing_html {
    ...
    my $video_embed_code = $self->get_video_embed_html($id, 'full', %params);
...
sub get_video_embed_html {
    my ($self, $id, $full_or_min, %params) = @_;
    ...
    my $thumbnail_url = $self->{'urls'}{'bbcparl-logo'};
    my $secs_offset = $self->{'programmes'}{$id}{'offset'} || 0;
    
    # No " in the embed_html, please
    my $embed_html = '';
    ...
    if ($self->{'programmes'}{$id}{'rights'} eq 'internet'
        ...
        if ($full_or_min eq 'full') {
            $embed_html .= "<object width='$width' height='$height' classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' codebase='http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0'>" .
...
sub valid_output_param {
    ...
    } qw (html js-full js-minimal js-list); # XXX xml); # No XML output currently
